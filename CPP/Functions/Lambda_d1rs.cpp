...
Matrix3d Lambda_d1rs( Vector3d &T, Vector3d &T1, Vector3d &t, Vector3d &t1, Vector3d &tr, Vector3d &ts, Vector3d &t1r, Vector3d &t1s, Vector3d &trs, Vector3d &t1rs )
{
	...
	/* this quantity is used in calculating m2-m12 (m[1] - m[11]). In total, this
	quantity is used 14 times. Precalculating this eliminates 13 additions */
	const double dTt_plus_1 = 1 + dTt;
	...
	const double Inv_dTt_plus_1 = 1/dTt_plus_1;
	...
	m[1] = (-6*((dT1t_plus_dTt1*dTtr*dTts)/pow(dTt_plus_1,4)))*CTt_CTtTranspose;
	...
	m[11] = (1/(dTt_plus_1))*(CTt*(CT1trs + CTt1rs).transpose()
