...
//void Element_KR(MatrixXd &P, MatrixXd &Q, RowVector2d &End_pt,RowVectorXd &Ele2Cp,RowVectorXd &knotVector,int order,RowVectorXd &weights,RowVectorXd &Mat,Matrix3d &A0,int ngp)
{
  ...
  Vector3d T0, temp_vector;
	temp_vector = A0.col(0); tangent(temp_vector, T0);  // tangent vector to the centreline in refrence triad.
//    cout << T0 <<endl;
...
//		cout<< xi << endl<< endl;
    ...
    N.row(i) = ders.row(0);
    dN1.row(i) = ders.row(1);
    dN2.row(i) = ders.row(2);
...
//        cout << Ben<<endl<< endl;cout << ben<<endl<< endl;cout << ben_r<<endl<< endl;
				...
				vec1 = (RT1*LT0T+RT*LT0T1)*A0.col(2);
        vec2 = RT*LT0T*A0.col(1);
				Tor(0) = vec1.dot(vec2);

				vec1 = (Rt1*LTt*RT*LT0T+Rt*LTt1*RT*LT0T+Rt*LTt*RT1*LT0T+Rt*LTt*RT*LT0T1)*A0.col(2);
        vec2 = Rt*LTt*RT*LT0T*A0.col(1);
				tor(0) = vec1.dot(vec2);
				tor_r(0) = tor_dr(LT0T,LTt,LT0T1,LTt1,LTtr,LTt1r,RT,Rt,RT1,Rt1,Rtr,Rt1r,A0,3,2);

        vec1 = (RT1*LT0T+RT*LT0T1)*A0.col(1);
        vec2 = RT*LT0T*A0.col(2);
				Tor(1) = vec1.dot(vec2);

        vec1 = (Rt1*LTt*RT*LT0T+Rt*LTt1*RT*LT0T+Rt*LTt*RT1*LT0T+Rt*LTt*RT*LT0T1)*A0.col(1);
        vec2 = Rt*LTt*RT*LT0T*A0.col(2);
        tor(1) = vec1.dot(vec2);
