...
using namespace Eigen;
 tuple<MatrixXd, MatrixXd>Element_KR_bs(MatrixXd &P, MatrixXd &Q, RowVector2d &End_pt,RowVectorXd &Ele2Cp,RowVectorXd &knotVector,int order,RowVectorXd &weights,RowVectorXd &Mat,Matrix3d &A0,int ngp)
...
//                F_Element(r)=F_Element(r)+(Fm+Fb+Ft)*J1*Gauss_W(i);
                F_Element(r)=F_Element(r)+(Fb)*J1*Gauss_W(i);
                for (k=0;k<nos;k++)
                {
                	for (k1=0;k1<dof;k1++)
                	{
...
//						    K_Element(r,s)=K_Element(r,s)+(Km+Kb+Kt)*J1*Gauss_W(i);
						    K_Element(r,s)=K_Element(r,s)+(Kb)*J1*Gauss_W(i);
...
//    cout << "Stiffness Matrix: "<< endl << K_Element << endl << endl;
    return make_tuple(F_Element,K_Element);
